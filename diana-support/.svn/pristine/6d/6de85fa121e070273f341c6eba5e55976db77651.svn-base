    ...
    const std::string& elevel, const::string& unit)
{
  ...
  Projection proj(grid.projection, grid.x_resolution, grid.y_resolution);
  Rectangle rect(0, 0, (grid.nx - 1) * grid.x_resolution, (grid.ny - 1)
      * grid.y_resolution);
  ...
  field->nx = grid.nx;
  field->ny = grid.ny;
  field->gridResolutionX = grid.x_resolution;
  field->gridResolutionY = grid.y_resolution;
  field->area = area;
  field->allDefined = false;
  field->producer = 88;
  field->gridnum = atoi(param.grid.c_str());
  field->dtype = 2;
 // field->vcoord = 2;
  ...
  if( !field->analysisTime.undef() && !time.undef()) {
    field->forecastHour = miutil::miTime::hourDiff(time, field->analysisTime);
  } else {
    gridinventory::Taxis taxis = getTaxis(reftime, param.key.taxis);
    if (not taxis.values.empty()) {
      const time_t tt = atoi(taxis.values.front().c_str());
      const miutil::miTime t(tt);
      field->forecastHour = miutil::miTime::hourDiff(time, t);
    } else {
      field->forecastHour = -32767;
    }
  }
  ...
  field->pressureLevel = field->level;
  return field;
}
