    ...
    std::vector<std::string>& errors)
{
  ...
  const std::string key_derivedzaxis = "derivedzaxis";
  ...
  for (int l = 0; l < nlines; l++) {
    ...
    for (size_t  i = 0; i < tokens.size(); i++) {
      ...
      if (stokens.size() == 2 )  {
        ...
        } else  if( stokens[0] == key_derivedzaxis ) {
          zaxis_info.derivedZaxis = stokens[1];
        }
      }
    }
...
// static member
bool FieldFunctions::splitFieldSpecs(std::string& paramName, std::string& levelName)
{

  std::string vcoordName, ecoordName, unit, elevel;
  bool use_standard_name;
  bool ok = splitFieldSpecs( paramName, vcoordName, ecoordName, levelName, use_standard_name, unit, elevel);
  levelName = miutil::to_lower(levelName);
  return ok;
}

// static member
bool FieldFunctions::splitFieldSpecs(std::string& paramName,
    std::string& vcoordName, std::string& ecoordName,
    std::string& levelName, bool& use_standard_name, std::string& unit, std::string& elevel)
{
  ...
  for (size_t i=1; i<vs.size(); i++) {
    ...
    if ( vp[0]=="level" ) {
      ...
      if ( vp.size()==2 ) {
        vcoordName= miutil::to_lower(vp[1]);
      } else {
        vcoordName.clear();
      }
...
const std::string FieldFunctions::getFunctionText(const Function f) const
{
  ...
  return std::string();
  /*
  if (functionText.count(f) == 0) {
    return std::string();
  }
  return functionText.at(f);
   */
}

bool FieldFunctions::fieldComputer(const std::string& fieldName,
    const FieldFunctions::Function function,
    ...
    const std::vector<Field*>& vfres, GridConverter & gc)
{
  ...
  for (int i = 0; i < ninp; i++) {
    ...
    if (!vfinput[i]->allDefined)
      allDefined = false;
    if (vfinput[i]->nx != nx || vfinput[i]->ny != ny)
      ok = false;
  }
  ...
  for (int i = 0; i < nout; i++) {
    ...
    unit = vfres[i]->unit;
    if (vfres[i]->nx != nx || vfres[i]->ny != ny)
      ok = false;
  }
  ...
  int imapr, icori, nsmooth;
  float *xmapr = 0, *ymapr = 0, *fcoriolis = 0;
  float dxgrid, dygrid;
  ...
  case f_thesat_plevel_th:
    ...
    if (ninp != 1 || nout != 1)
      break;
    plevel = vfinput[0]->pressureLevel;
  ...
  case f_thmean_plevel_z1_z2:
    ...
    if (ninp != 2 || nout != 1)
      break;
    plevel1 = vfinput[0]->pressureLevel;
    plevel2 = vfinput[1]->pressureLevel;
  ...
  case f_qvector_plevel_z_th_ycomp:
    ...
    imapr = 3;
    icori = 1;
    if (gc.getMapFields(vfinput[0]->area, imapr, icori, nx, ny, &xmapr, &ymapr,
        &fcoriolis, dxgrid, dygrid))
  ...
  case f_kindex_plevel_th_rh:
    ...
    plevel1 = vfinput[0]->pressureLevel;
    plevel2 = vfinput[1]->pressureLevel;
    plevel3 = vfinput[3]->pressureLevel;
  ...
  case f_boydenindex_plevel_th_z:
    ...
    if (ninp != 3 || nout != 1)
      break;
    plevel1 = vfinput[1]->pressureLevel;
    plevel2 = vfinput[2]->pressureLevel;
...
#pragma omp parallel for
  ...
  case f_min_value:
    res = minValue(nx, ny, finp, fout[0],
        allDefined, undef);
    break;

  case f_max_value:
    res = maxValue(nx, ny, finp, fout[0],
        allDefined, undef);
    break;
  ...
  case f_probability_above:
    if (nout != 1 || nconst < 1)
      break;
    res = probability(true, nx, ny, finp, constants, fout[0],
        allDefined, undef);
    break;
  ...
  case f_probability_below:
    if (nout != 1 || nconst < 1)
      break;
    res = probability(false, nx, ny, finp, constants, fout[0],
        allDefined, undef);
    break;
  ...
  if (allDefined) {
    ...
    if (compute < 3 ) {
      for (int i = nx; i < fsize - nx; i++) {
        dugdx = xmapr[i] * (ug[i + 1] - ug[i - 1]);
        dvgdx = xmapr[i] * (vg[i + 1] - vg[i - 1]);
        dtdx = xmapr[i] * tscale * (t[i + 1] - t[i - 1]);
        dtdy = ymapr[i] * tscale * (t[i + nx] - t[i - nx]);
        qcomp[i] = c * (dugdx * dtdx + dvgdx * dtdy);
      }
    } else {
      for (int i = nx; i < fsize - nx; i++) {
        dugdy = ymapr[i] * (ug[i + nx] - ug[i - nx]);
        dvgdy = ymapr[i] * (vg[i + nx] - vg[i - nx]);
        dtdx = xmapr[i] * tscale * (t[i + 1] - t[i - 1]);
        dtdy = ymapr[i] * tscale * (t[i + nx] - t[i - nx]);
...
#pragma omp parallel for
  for (int i = nx; i < fsize - nx; i++) {
    ...
    if (calculations::is_defined(allDefined, z[i-nx], z[i-1], z[i+1], z[i+nx], undef))
      ug[i] = -ymapr[i] * (z[i+nx] - z[i-nx]) * g / fcoriolis[i];
    else
      ug[i] = undef;
  }
...
#pragma omp parallel for
  for (int i = nx; i < fsize - nx; i++) {
    ...
    if (calculations::is_defined(allDefined, z[i-nx], z[i-1], z[i+1], z[i+nx], undef))
      vg[i] = xmapr[i] * (z[i + 1] - z[i - 1]) * g / fcoriolis[i];
    else
      vg[i] = undef;
  }
    ...
    const float *xmapr, const float *ymapr, const float *fcoriolis, bool& allDefined, float undef)
{
  ...
  if (allDefined) {
    ...
    // loop extended, reset bad computations at boundaries later
    for (int i = nx; i < fsize - nx; i++)
      gvort[i] = (xmapr[i] * xmapr[i] * (z[i - 1] - 2. * z[i] + z[i + 1])
          + ymapr[i] * ymapr[i] * (z[i - nx] - 2. * z[i] + z[i + nx])) * g4
          / fcoriolis[i];
    ...
    float undef)
{
  ...
  if (allDefined) {
    // loop extended, reset bad computations at boundaries later
    for (int i = nx; i < fsize - nx; i++) {
      ug[i] = -ymapr[i] * (mpot[i + nx] - mpot[i - nx]) / fcoriolis[i];
      vg[i] = xmapr[i] * (mpot[i + 1] - mpot[i - 1]) / fcoriolis[i];
    }
  } else {
    for (int i = nx; i < fsize - nx; i++) {
...
#pragma omp parallel for
  for (int i = nx; i < fsize - nx; i++) {
    if (calculations::is_defined(allDefined, v[i-1], v[i+1], u[i-nx], u[i+nx], undef))
      rvort[i] = xmapr[i] * (v[i+1] - v[i-1]) - ymapr[i] * (u[i+nx] - u[i-nx]);
    else
      rvort[i] = undef;
  }
...
#pragma omp parallel for
  for (int i = nx; i < fsize - nx; i++) {
    if (calculations::is_defined(allDefined, v[i-1], v[i+1], u[i-nx], u[i+nx], undef))
      avort[i] = xmapr[i] * (v[i+1] - v[i-1]) - ymapr[i] * (u[i+nx] - u[i-nx]) + fcoriolis[i];
    else
      avort[i] = undef;
  }
...
#pragma omp parallel for
  for (int i = nx; i < fsize - nx; i++) {
    if (calculations::is_defined(allDefined, v[i-1], v[i+1], u[i-nx], u[i+nx], undef))
      diverg[i] = xmapr[i] * (u[i+1] - u[i-1]) + ymapr[i] * (v[i+nx] - v[i-nx]);
    else
      diverg[i] = undef;
  }
...
#pragma omp parallel for
  for (int i = nx; i < fsize - nx; i++) {
    if (calculations::is_defined(allDefined, u[i], v[i], f[i-nx], f[i-1], f[i+1], f[i+nx], undef))
      advec[i] = (u[i] * xmapr[i] * (f[i+1] - f[i-1]) + v[i] * ymapr[i] * (f[i+nx] - f[i-nx])) * scale;
    else
      advec[i] = undef;
  }
    ...
    float *fgrad, const float *xmapr, const float *ymapr, bool& allDefined, float undef)
{
  ...
  case 1: // df/dx (loop extended, reset/complete later
    if (allDefined) {
      for (int i = 1; i < fsize - 1; i++)
        fgrad[i] = xmapr[i] * (field[i + 1] - field[i - 1]);
    } else {
      for (int i = 1; i < fsize - 1; i++) {
  ...
  case 2: // df/dy (loop extended, reset/complete later
    if (allDefined) {
      for (int i = nx; i < fsize - nx; i++)
        fgrad[i] = ymapr[i] * (field[i + nx] - field[i - nx]);
    } else {
      for (int i = nx; i < fsize - nx; i++) {
  ...
  case 3: // abs(del(f))= sqrt((df/dx)**2 + (df/dy)**2)
    if (allDefined) {
      for (int i = nx; i < fsize - nx; i++) {
        dfdx = xmapr[i] * (field[i + 1] - field[i - 1]);
        dfdy = ymapr[i] * (field[i + nx] - field[i - nx]);
  ...
  case 4: // delsquare(f)= del(del(f))
    if (allDefined) {
      for (int i = nx; i < fsize - nx; i++) {
        ...
        fgrad[i] = 4.0f * (xmapr[i] * xmapr[i] * d2fdx + ymapr[i] * ymapr[i]
                                                                          * d2fdy);
      }
    } else {
      for (int i = nx; i < fsize - nx; i++) {
        if (field[i - nx] != undef && field[i - 1] != undef && field[i]
          ...
          d2fdx = field[i - 1] - 2.0 * field[i] + field[i + 1];
          d2fdy = field[i - nx] - 2.0 * field[i] + field[i + nx];
          fgrad[i] = 4.0 * (xmapr[i] * xmapr[i] * d2fdx + ymapr[i] * ymapr[i]
                                                                           * d2fdy);
        } else
          fgrad[i] = undef;
      }
    ...
    float *tfp, const float *xmapr, const float *ymapr, bool& allDefined, float undef)
{
  ...
  if (allDefined) {
    for (int i = nx; i < fsize - nx; i++) {
      if (absdelt[i] != 0.0) {
        dabsdeltdx = xmapr[i] * (absdelt[i + 1] - absdelt[i - 1]);
        dabsdeltdy = ymapr[i] * (absdelt[i + nx] - absdelt[i - nx]);
        dtdxa = xmapr[i] * (tx[i + 1] - tx[i - 1]) / absdelt[i];
        dtdya = ymapr[i] * (tx[i + nx] - tx[i - nx]) / absdelt[i];
  ...
  if (allDefined) {
    for (int i = 0; i < fsize; i++) {
      ...
      if (fcor >= 0. && fcor < fcormin)
        ...
        fcor = fcormax;
      mxy[i] = float(i % nx) + v[i] * 2.0 * xmapr[i] / fcor;
    }
  } else {
    for (int i = 0; i < fsize; i++) {
      if (v[i] != undef) {
  ...
  if (allDefined) {
    for (int i = 0; i < fsize; i++) {
      ...
      if (fcor >= 0. && fcor < fcormin)
        ...
        fcor = fcormax;
      nxy[i] = float(i / nx) - u[i] * 2.0 * ymapr[i] / fcor;
    }
  } else {
    for (int i = 0; i < fsize; i++) {
      if (u[i] != undef) {
    ...
    float *fjacobian, const float *xmapr, const float *ymapr, bool& allDefined, float undef)
{
  ...
  if (allDefined) {
    ...
    // loop extended, reset bad computations at boundaries later
    for (int i = nx; i < fsize - nx; i++) {
      df1dx = xmapr[i] * (field1[i + 1] - field1[i - 1]);
      df1dy = ymapr[i] * (field1[i + nx] - field1[i - nx]);
      df2dx = xmapr[i] * (field2[i + 1] - field2[i - 1]);
      df2dy = ymapr[i] * (field2[i + nx] - field2[i - nx]);
...
// static
bool FieldFunctions::minValue(int nx, int ny, const vector<float*>& fields,
    float *fres, bool& allDefined, float undef)
{
...
// static
bool FieldFunctions::maxValue(int nx, int ny, const vector<float*>& fields,
    float *fres, bool& allDefined, float undef)
{
  // returns max value in each grid point
#ifdef ENABLE_FIELDFUNCTIONS_TIMING
  METLIBS_LOG_TIME();
#endif
...
// static
bool FieldFunctions::probability(bool above, int nx, int ny, const vector<float*>& fields,
    const vector<float>& limits, float *fres, bool& allDefined, float undef)
{
  // returns probability of  value above/below limit in each grid point
#ifdef ENABLE_FIELDFUNCTIONS_TIMING
  METLIBS_LOG_TIME();
#endif
  //TODO:: allDefined
...
logit(t)=(1-e^(t-274.5)3.5)/(1+e^(t-274.3)3.5 )

mm2cm(t)=0.13/(0.02+0.01((t-252.0)/20)^2 )
...
#pragma omp parallel for shared(allDefined)
  for (int i = 0; i < fsize; i++) {	
    if (calculations::is_defined(inAllDefined, snow_water[i], tk2m[i], td2m[i], undef)) {
      ...
      float logit_t=(1-exp((t-274.5)*3.5))/(1+exp((t-274.3)*3.5) );

      float mm2cm_t=0.13/(0.02+0.01*((t-252.0)/20.0)*((t-252.0)/20.0));
