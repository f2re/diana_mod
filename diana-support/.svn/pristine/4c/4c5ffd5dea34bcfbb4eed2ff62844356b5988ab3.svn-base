...
public:
  ...
  struct Zaxis_info {
    ...
    std::string derivedZaxis;
   Zaxis_info() : vctype(vctype_none)
  {
  }
  };
...
public:
  ...
  static bool splitFieldSpecs(std::string& paramName,
  ...
  const std::string getFunctionText(const Function f) const;
  ...
  bool isTimeStepFunction(const Function f) const
  {
    return (f==f_field_diff_forecast_hour || f==f_accum_diff_forecast_hour || f==f_sum_of_forecast_hours);
  }

  /// call function and return results
  bool fieldComputer(const std::string& fieldName,
      const Function function, const std::vector<float>& constants,
  ...
  static bool minValue(int nx, int ny, const std::vector<float*>& fields,
      float *fres, bool& allDefined, float undef);

  static bool maxValue(int nx, int ny, const std::vector<float*>& fields,
      float *fres, bool& allDefined, float undef);

  static bool probability(bool above, int nx, int ny, const std::vector<float*>& fields,
