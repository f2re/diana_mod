...
#if (MIFI_VERSION_MAJOR > 0) or (MIFI_VERSION_MINOR >= 50)
...
const char VCROSS_BOUNDS[] = "vcross_bnds";
const double LONLAT_TOLERANCE = 1e-4;
...
size_t FimexIO::findTimeIndex(const gridinventory::Taxis& taxis, const miutil::miTime& time)
{
  ...
  if (not time.undef()) {
    ...
    vector<std::string>::const_iterator titr = taxis.values.begin();
    for (; titr != taxis.values.end(); ++titr, taxis_index++) {
      time_t tt = atoi((*titr).c_str());
      miTime t(tt);
      if (t == time) {
        break;
      }
...
size_t FimexIO::findZIndex(const gridinventory::Zaxis& zaxis, const std::string& zlevel)
{
  ...
  if (not (zlevel.empty() and zaxis.values.size() == 1)) {
    if (!zaxis.positive) {
      vector<std::string>::const_iterator zitr = zaxis.values.begin();
      for (; zitr != zaxis.values.end(); ++zitr, zaxis_index++) {
        if (*zitr == zlevel) {
          break;
        }
      }
    } else {
      vector<string>::const_reverse_iterator zitr = zaxis.values.rbegin();
      for (; zitr != zaxis.values.rend(); ++zitr, zaxis_index++) {
        if (*zitr == zlevel) {
          break;
        }
      }
...
size_t FimexIO::findExtraIndex(const gridinventory::ExtraAxis& extraaxis, const std::string& elevel)
{
  ...
  vector<string>::const_iterator ritr = extraaxis.values.begin();
  for (; ritr != extraaxis.values.end(); ++ritr, extraaxis_index++) {
    if (*ritr == elevel) {
      break;
    }
  }
...
FimexIO::FimexIO(const std::string & modelname, const std::string & sourcename,
    ...
    const std::vector<std::string>& options, FimexIOsetup * s) :
...
namespace /* anonymous */ {
struct y_lon_lat {
  std::string yaxis_name, lon_name, lat_name;
};
typedef std::map<std::string, y_lon_lat> x2y_lon_lat_t;
...
void addTimeAxis(CoordinateSystem::ConstAxisPtr tAxis, const std::vector<std::string>& values,
    name2id_t& name2id, const std::string& rtime, gridinventory::Inventory& inventory)
{
  const std::string& name = tAxis->getName(), id = makeId(name, values.size());
  name2id[name] = id;
  gridinventory::Taxis taxis = gridinventory::Taxis(id, name, values);
  ...
  if (utmpos != std::string::npos) {
    ...
    for (size_t ii=0; ii<tokens.size(); ++ii) {
      ...
      if (stokens.size() == 2 && stokens[0] == "+zone") {
        ...
        utmstr += stokens[1];
        ostringstream ost;
        ost <<"+proj=tmerc +lon_0="<<lon_0<<" +lat_0=0 +k=0.996 +x_0=500000";
    ...
    CoordinateSystem::ConstAxisPtr vAxis, CoordinateSystemPtr& cs)
{
  ...
  int vc_type = FieldFunctions::vctype_other;
  ...
  if (positive) {
    for (int i = vdata->size() - 1 ; i>-1; --i) {
      ostringstream ost;
      ost << levelprefix<<idata[i]<<levelsuffix;
      levels.push_back(ost.str());
    }
  } else {
  ...
  zaxes.insert(gridinventory::Zaxis(id, vAxis->getName(), vc_type, positive, levels,"",verticalType));
...
bool FimexIO::makeInventory(const std::string& reftime)
{
  ...
  set<gridinventory::VCrossSection> verticalCrossSections;
  ...
  try {
        ...
        if (cs->hasAxisType(CoordinateAxis::ReferenceTime)) { //has reference time axis
          ...
          for (size_t i = 0; i < refTimes->size(); ++i) {
            ...
            for (size_t j = 0; j < times_offset->size(); ++j) {
              ostringstream ost;
              ost << timeOffsetData[j];
              values.push_back(ost.str());
            }
            
            if (not singleTimeStep) {
              addTimeAxis(tAxis, values, name2id, referenceTimes[i], inventory);
              values.clear();
            }
          }
          
          if (singleTimeStep and not referenceTimes.empty())
            addTimeAxis(tAxis, values, name2id, referenceTimes[0], inventory);
          ...
          boost::shared_array<int> timedata = tdata->asInt();
    ...
    for (size_t j = 0; j < nvars; ++j) {
      ...
      if (CDMparamName == VCROSS_NAME or CDMparamName == VCROSS_BOUNDS)
        continue;
      ...
      if ((param.standard_name == LONGITUDE or param.standard_name == LATITUDE) and shape.size()==2) {
        // FIXME somehow xaxisname and yaxisname are empty for latitude/longitude from fimex vertical cross section file

        const std::string &xname = shape[0], &yname = shape[1]; // FIXME do not use shape!
        METLIBS_LOG_DEBUG(LOGVAL(xname) << LOGVAL(yname));
        y_lon_lat& yll = x2y_lon_lat[xname];
        yll.yaxis_name = yname;
        if (param.standard_name == LONGITUDE)
          yll.lon_name = paramname;
        else
          yll.lat_name = paramname;
      }

      parameters.insert(param);
    }

    mVcrossIndices.clear();

    BOOST_FOREACH(const x2y_lon_lat_t::value_type& xyll, x2y_lon_lat) {
      const std::string &lon = xyll.second.lon_name, &lat = xyll.second.lat_name;
      if (lon.empty() or lat.empty())
        continue;

      const std::string &xname = xyll.first, &yname = xyll.second.yaxis_name;
      const DataPtr dataX = feltReader->getScaledData(xname);
      const DataPtr dataY = feltReader->getScaledData(yname);
      if (not (dataX and dataY and dataY->size() == 1))
        continue;

      const DataPtr dataLon = feltReader->getScaledData(lon);
      const DataPtr dataLat = feltReader->getScaledData(lat);
      ...
      if (cdm.hasVariable(VCROSS_NAME) and cdm.hasVariable(VCROSS_BOUNDS)) {
        const CDMVariable &varVCNames = cdm.getVariable(VCROSS_NAME);
        ...
        for (size_t ivc=0; ivc<nvcross; ++ivc) {
          SliceBuilder sb_bounds(cdm, VCROSS_BOUNDS);
          sb_bounds.setStartAndSize(vc_shape.back(), ivc, 1);
          DataPtr dataBounds = feltReader->getScaledDataSlice(VCROSS_BOUNDS, sb_bounds);
          if (dataBounds->size() != 2) {
            METLIBS_LOG_WARN(VCROSS_BOUNDS << " size != 2");
            continue;
          }
          boost::shared_array<int> valuesBounds = dataBounds->asInt();
          const size_t lola_begin = valuesBounds[0], lola_end = valuesBounds[1]+1;
//          if (lola_end - lola_begin == 1)
//            // vertical profile, not a cross section
//            continue;

          const std::vector<float> lons(vLon.get() + lola_begin, vLon.get() + lola_end);
          const std::vector<float> lats(vLat.get() + lola_begin, vLat.get() + lola_end);

          SliceBuilder sb_name(cdm, VCROSS_NAME);
          sb_name.setStartAndSize(vc_shape.back(), ivc, 1);
          DataPtr dataName = feltReader->getScaledDataSlice(VCROSS_NAME, sb_name);
          boost::shared_array<char> valuesName = dataName->asChar();
          const std::string csname(valuesName.get());

          gridinventory::VCrossSection vc("vc", csname, xname, yname, lons, lats);
          verticalCrossSections.insert(vc);
          mVcrossIndices.insert(std::make_pair(vc.name, VcrossBeginEnd(lola_begin, lola_end)));
          METLIBS_LOG_DEBUG("added segment VCrossSection '" << csname << "' as '" << vc.sortkey() << "'");
        }
      } else {
        const std::vector<float> lons(vLon.get(), vLon.get() + dataLon->size());
        const std::vector<float> lats(vLat.get(), vLat.get() + dataLat->size());

        gridinventory::VCrossSection vc("vc", "vc", xname, yname, lons, lats);
        verticalCrossSections.insert(vc);
        mVcrossIndices.insert(std::make_pair("", VcrossBeginEnd(0, dataLon->size())));
        METLIBS_LOG_DEBUG("added single VCrossSection '" << vc.sortkey() << "'");
      }
    }
    ...
    for (gridinventory::Inventory::reftimes_t::iterator it_ri = rtimes.begin(); it_ri != rtimes.end(); ++it_ri) {
      ...
      ri.verticalCrossSections = verticalCrossSections;
      ri.timestamp = now;
    }
    ...
    const std::string& elevel, const std::string& unit)
{
  ...
  try {
    ...
    const size_t dataSize = data->size(), fieldSize = field->nx * field->ny;
    if (dataSize != fieldSize) {
      ...
      copyFieldSwapY(grid.y_direction_up, field->nx, field->ny, &fdata[0], field->data);
    ...
    const std::string& output_time)
{
  ...
  try {
    ...
    const size_t fieldSize = field->nx * field->ny;
    boost::shared_array<float> fdata(new float[fieldSize]);

    mifi_bad2nanf(&fdata[0], &fdata[0]+fieldSize, fieldUndef);

    const gridinventory::Grid grid = getGrid(reftime, param.grid);
    copyFieldSwapY(grid.y_direction_up, field->nx, field->ny, field->data, &fdata[0]);
