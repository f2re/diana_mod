...
{
  //cerr << "FieldCache::flush: before maximumsize_" << maximumsize_ << ",bytesize_ " << bytesize_ << ", mapsize" << fields.size() << endl;
  ...
  while(itr!=fields.rend()) {
    try {
      //cerr << "FieldCache::flush: before erase maximumsize_" << maximumsize_ << ",bytesize_ " << bytesize_ << ", mapsize" << fields.size() << endl;
      // delete the field and the key
      erase(itr->first);
      //cerr << "FieldCache::flush: after erase maximumsize_" << maximumsize_ << ",bytesize_ " << bytesize_ << ", mapsize" << fields.size() << endl;
      // set itr to the end
      itr=fields.rbegin();
    }
      ...
      itr++;
      cerr << "flush() " << e.what() << endl;
    }
  }
  //cerr << "FieldCache::flush: after maximumsize_" << maximumsize_ << ",bytesize_ " << bytesize_ << ", mapsize" << fields.size() << endl;
...
bool FieldCache::parseSetup(vector<std::string> lines,
    vector<std::string>& errors)
{
#ifdef DEBUGPRINT
  cerr << "FieldCache::parseSetup()"<<endl;
#endif
  //MI_LOG & log = MI_LOG::getInstance(PKG".parseSetup");
  std::string key;
...
{
  return (fields.count(keyset) ? fields[keyset].get() : 0);
}
...
throw(ModifyFieldCacheException&)
{
  ...
  METLIBS_LOG_DEBUG("copied " << keyset << " to " << newk);
}
...
throw(ModifyFieldCacheException&)
{
  ...
  METLIBS_LOG_DEBUG("erase() " << keyset);
}
...
void FieldCache::freeField(Field* f) throw(ModifyFieldCacheException&)
  ...
  if(!f->isPartOfCache) {
    METLIBS_LOG_DEBUG("freeField() Uncached Field: " << f->modelName << " " << f->paramName
...
throw(ModifyFieldCacheException&)
{
  if(st == FieldCache::KILOBYTE ) s*=1024;
  if(st == FieldCache::MEGABYTE ) s*=1024*1024;
...
{
  if(st == FieldCache::KILOBYTE ) return maximumsize_/1024;
  if(st == FieldCache::MEGABYTE ) return maximumsize_/(1024*1024);
  return maximumsize_;
}
