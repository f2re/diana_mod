/*
 ...
 $Id: glTextTTPixmap.cc 3117 2010-03-02 12:57:17Z dages $
...
// glTextTTPixmap.cc : Class definition for GL-rendered truetype-fonts
...
#include <fstream>
#include "glTextTTPixmap.h"
#include <iostream>
#include <miFTGL/FTGLTextureFont.h>
#include <miFTGL/FTGLPixmapFont.h>
...
glTextTTPixmap::glTextTTPixmap() :
  glText(), scalex(1.0), scaley(1.0), xscale(1.0)
{
#ifdef DEBUGPRINT
  cerr << "glTextTTPixmap::constructor" << endl;
#endif
  initialised = true;
}
...
bool glTextTTPixmap::testDefineFonts(string path)
{
  string famn;
  string fontn;
  if (path.length() == 0)
...
bool glTextTTPixmap::defineFont(const string family, const string fname,
...
bool glTextTTPixmap::set(const string fam, const FontFace face,
    const float size)
{
...
bool glTextTTPixmap::setFont(const string family)
{
  int fidx;
  if (_findFamily(family, fidx)) {
    FontIndex = fidx;
...
bool glTextTTPixmap::setFontFace(const FontFace face)
{
  Face = face;
  bool b = _calcScaling();
  if (hardcopy)
...
bool glTextTTPixmap::setFontSize(const float size)
{
  reqSize = size * 1.3;
  bool b = _calcScaling();
  if (hardcopy)
...
void glTextTTPixmap::setScalingType(const FontScaling fs)
{
  scaletype = fs;
  _calcScaling();
}

bool glTextTTPixmap::_calcScaling()
{
...
void glTextTTPixmap::setGlSize(const float glw, const float glh)
{
  glWidth = glw;
  glHeight = glh;
...
void glTextTTPixmap::setVpSize(const float vpw, const float vph)
{
  vpWidth = vpw;
  vpHeight = vph;
...
bool glTextTTPixmap::_checkFont()
{
  ...
  if (!tf->fontname.size()) {
    //cerr << "glTextTTPixmap: zero fontname" << endl;
    return false;
  }
  if (!(tf->created)) {
    cerr << "glTextTTPixmap: Creating font for Family:"
        << FamilyName[FontIndex] << " Face:" << facenames[Face] << " Size:"
        << Sizes[SizeIndex] << endl;
    tf->pfont = new FTGLPixmapFont(tf->fontname.c_str());
    FT_Error error = tf->pfont->Error();
    if (error != 0) {
      cerr << "glTextTTPixmap:ERROR creating font from:" << tf->fontname
          << endl;
      delete tf->pfont;
      tf->pfont = 0;
      return false;
...
bool glTextTTPixmap::drawChar(const int c, const float x, const float y,
    const float a)
{
  char s[2];
  s[0] = (char) c;
...
bool glTextTTPixmap::drawStr(const char* s, const float x, const float y,
    const float a)
{
  if (!s)
    return false;
...
bool glTextTTPixmap::getStringSize(const char* s, float& w, float& h)
{
  if (!s) {
    w = 0;
    h = 0;
...
bool glTextTTPixmap::getCharSize(const int c, float& w, float& h)
{
  char s[2];
  s[0] = (char) c;
  s[1] = '\0';
...
bool glTextTTPixmap::getMaxCharSize(float& w, float& h)
{
  getCharSize('M', w, h);
  return true;
}

string glTextTTPixmap::getPsName() const
{
